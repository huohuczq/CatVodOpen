...
async function home(filter) {
    const classes = [{'type_id':1,'type_name':'热门搜索'},{'type_id':2,'type_name':'资源列表'}];
    const filterObj = {};
    return {
        class: classes,
        filters: filterObj,
...
async function category(inReq, _outResp) {
    ...
    const videos = $(`.sou-con-list:eq(${idx}) > ul > li`)
        .map((_, li) => {
            return {
                vod_id: $(li).find('a').eq(3-id).attr('href'),
...
async function detail(inReq, _outResp) {
    ...
    for (const id of ids) {
        ...
        const $ = load(resp);
        const shareUrls = $('.umCopyright').find('a:eq(0)').attr('href');
        let vod = ({
            vod_id: id,
        });
